...
pub async fn share_program(programid: i32, app: tauri::AppHandle) {
    ...
    let json = serde_json::to_string(&ProgramWithWorkouts {
        program,
        days,
        image,
    })
...
pub async fn restore_program(app: tauri::AppHandle) -> Option<Program> {
    ...
    if let Some(path) = path {
        restore_program_from_path(path, app).await
    } else {
        None
    }
}

pub async fn restore_program_from_path(path: FilePath, app: tauri::AppHandle) -> Option<Program> {
    let path = path.as_path().unwrap();
    ...
    let image_path = if let Some(image) = program_with_workouts.image {
        ...
        let image = decoder.decode().expect("Error decoding image");
    ...
    let new_program = diesel::insert_into(programs::table)
        .values((
            programs::dsl::title.eq(program_with_workouts.program.title),
